...
const SessionController = {
  ...
  getSessionById: async (req, res) => {
    ...
    try {
      const session = await Session.findByPk(id, { include: [User, Computer] });
      if (!session)
        return res.status(404).json({ message: "Sesi tidak ditemukan" });
  ...
  updateSession: async (req, res) => {
    ...
    try {
      ...
      res.json({ message: "Sesi berhasil diperbarui", session });
    } catch (error) {
      res
        .status(500)
        .json({ message: "Gagal memperbarui sesi", error: error.message });
    }
  },
  ...
  deleteSession: async (req, res) => {
    ...
    try {
      ...
      // Hanya izinkan menghapus sesi yang belum selesai
      if (session.status === "finished") {
        return res
          .status(400)
          .json({ message: "Tidak dapat menghapus sesi yang sudah selesai" });
      }
      ...
      res
        .status(500)
        .json({ message: "Gagal menghapus sesi", error: error.message });
    }
  },
};
